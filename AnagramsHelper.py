...
class MyWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Anagrams Helper")
    ...
    def initUI(self, layout):
        font = 'Georgia'
        self.title = QLabel("Anagrams Helper")
        self.title.setAlignment(Qt.AlignHCenter)
        self.title.setFont(QFont('Georgia', 25))

        self.author = QLabel("by: Rishabh Kanodia")
    ...
    def getBoard(self):
        ...
        # 2d array board
        for i in range(6):
                self.board[i] = self.letterDistribution[np.random.randint(len(self.letterDistribution))]
                self.boardletters += self.board[i]
        print('Board:\n', self.board)

        # Print the board to the GUI
        row1 = str(self.board)
        row1 = filter(str.isalpha,row1)
        row1 = " ".join(row1)
    ...
    def insertBoard(self):
        ...
        self.boardletters, ok = QInputDialog.getText(self, 'Board Input', 'Enter the board as a 6-letter string in all caps: ')
        self.boardletters = self.boardletters.upper()
        print(self.boardletters)
        if ok and len(self.boardletters) == 6 and self.boardletters.isalpha():
            for i in range(6):
                    self.board[i] = self.boardletters[i]
            print('Board:\n', self.board)
        ...
        elif len(self.boardletters) != 6:
            self.printer.setText('Please input a 6 letter string')
    ...
    def firstFilter(self):
        ...
        self.initFilter = set(self.initFilter)
        self.initFilter = sorted(self.initFilter, key=len)
        print('Words Found:', len(self.initFilter))
        print(self.initFilter)
        for word in self.initFilter:
            self.output.append(str(word))
        return self.initFilter
